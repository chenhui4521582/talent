...
import {
  Card, Divider, notification, Modal, DatePicker, Form, Tooltip, Row, Col, Input
} from 'antd';
...
export default (props) => {
  ...
  const columns: ColumnProps<any>[] = [{
    ...
    render: (_, record) => {
      if (record.pushStatus === 1) {
        return(
          <span>
            <Link to={`/talent/resume/cvdetails?resumeId=${record.resumeId}&resumeStatus=${record.downStatus}`}>
              查看简历
            </Link>
  ...
  const handleSetTime = async (values) => {
    ...
    if(res.status === 200) {
      ...
      history.push(`/talent/resume/cvupload?undownResumeId=${curRecord.resumeId}&jobId=${curRecord.positionId}`);
      notification['error']({
        message: res.msg,
        description: '',
      });
