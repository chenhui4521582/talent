...
import {
  Card, Divider, notification,Form, Row, Col, Input, Modal
} from 'antd';
...
export default (props) => {
  ...
  const columns: ColumnProps<any>[] = [{
    ...
    render: (_, record) => {
      return(
        <span>
          <Link to={`/talent/resume/cvdetails?resumeId=${record.resumeId}&resumeStatus=${record.status}`}>
            查看简历
          </Link>
          <Divider type="vertical" />
          {
            record.pushStatus === 0 ?
            <a onClick={e => handleSendResume(record.status, record.resumeId)}>推送</a>
            :
            <span>已推送</span>
          }
        </span>
  ...
  const handleSendResume = async (downStatus: number, resumeIds: string) => {
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk: async () => {
        let res: GlobalResParams<string> = await sendResume({ downStatus, resumeIds, demandId});
        if(res.status === 200) {
          refresh();
