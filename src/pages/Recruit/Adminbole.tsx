...
import { Card, Form, Select, Row, Col, notification, Modal, Table } from 'antd';
...
export default props => {
  ...
  const columns: ColumnProps<IBoleTableItem>[] = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return (
          <a onClick={e => showModal(record.demandId)}>
            {text}/{record.boleRecommendNumber}
          </a>
        );
      },
    },
    ...
    },
    {
      ...
      render: (_, record) => {
        if (record.status === 1) {
          return <a onClick={e => handleRemoveBole(record.demandId)}>撤销</a>;
        } else {
          return null;
        }
      },
  ...
  const { TableContent, refresh } = useTable({
    ...
    columns,
    rowKeyName: 'demandId',
    cacheKey: props.queryMethod ? '伯乐奖管理1' : '伯乐奖管理2',
  });
