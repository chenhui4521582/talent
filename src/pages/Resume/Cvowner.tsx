...
import { Card, Divider, Modal, notification,Form, Row, Col, Select, Input, DatePicker } from 'antd';
import { history } from 'umi';
import moment from 'moment';
import { useTabTable } from '@/components/GlobalTable/useTabTable';
import {
  queryMyResume, IResumeTable, removeResume, selectDemand, IDemandParams, listResumeRecord, IChangeRecords,
  updateResumeStatus, IUpdateResume
...
export default (props) => {
  ...
  const handleClick = (record: IResumeTable) => { // 查看简历详情
    history.push(`/talent/resume/print?resumeId=${record.resumeId}&resumeStatus=${record.status}`);
  };
  const handleEdit = (record: IResumeTable) => { // 编辑简历
    history.push('/talent/Resume/cvHandupload?resumeId='+record.resumeId+'&resumeStatus='+record.status);
  };
  const handleshow = async (record: IResumeTable, type: string) => {
    if (type === 'change') {
      ...
      let res: GlobalResParams<IChangeRecords[]> = await listResumeRecord(record.resumeId);
      setChangeRecords(res.obj);
    }
    setCurRecord(record);
    setModalName(type);
  };
  const handleDelete = (record: IResumeTable) => { // 删除简历
    Modal.confirm({
      title: '确认要删除简历?',
      okText: '确定',
  ...
  const handleChangeStatus = async () => {
    ...
    data.interviewTime && (data.interviewTime = moment(data.interviewTime).format('YYYY-MM-DD HH:mm:ss'));
    let res: GlobalResParams<string> = await updateResumeStatus(data as IUpdateResume);
    if (res.status === 200) {
      refresh();
      modalCancel();
      notification['success']({
    ...
    <Card title={props.name ? props.name : "我的简历"}>
          ...
          <Row>
            <Col span={6}>
              <Form.Item
                ...
                name="degree"
              >
                <Select showSearch optionFilterProp='children' placeholder="请选择学历">
                  {
                    Object.keys(ResumeStatus).map(item => {
                      return <Option value={item} key={item}>{ResumeStatus[item]}</Option>
                    })
                  }
                </Select>
              </Form.Item>
            ...
            <Col span={6} offset={2}>
              ...
              >
                <Select placeholder="请选择面试状态" showSearch optionFilterProp='children'>
      ...
      >
        ...
        >
          ...
          >
            <Select placeholder="请选择招聘需求" showSearch optionFilterProp='children'>
              {
                demands?.map(item => {
                  return <Option value={item.demandId} key={item.demandId}>{item.businessLineName}--{item.jobName}</Option>
                })
              }
            </Select>
          </Form.Item>
          ...
          >
            <Select placeholder="请选择变更的状态" showSearch optionFilterProp='children' onChange={(value: number) => setCurStatus(value)}>
          ...
          </Form.Item>
          {
            ...
            <Form.Item
              label="面试时间"
              name="interviewTime"
              rules= {[{ required: curStatus === 2 ? true : false, message: '请选择面试时间' }]}
            >
      ...
      >
        <div>
        {
          changeRecords.length > 0 ? changeRecords.map(item => {
