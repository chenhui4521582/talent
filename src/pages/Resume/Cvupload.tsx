...
export default props => {
  ...
  const customRequest = async files => {
    ...
    let list = {
      ...
      percent: 100,
      uploadStatus:
        res.status === 200 ? '上传成功' : '上传失败（' + res.msg + '）',
  ...
  const manualClick = () => {
    if (undownResumeId) {
      history.push(
        `/Talent/resume/cvHandupload?undownResumeId=${undownResumeId}`,
      );
    } else {
  ...
  const checkDetail = (resumeId: number) => {
    window.open(
      '/talent/Resume/Cvdetails?resumeId=' + resumeId + '&resumeStatus=2',
    );
  };
        ...
        <Row>
          <Col span={10} offset={5}>
            <Select
              ...
              onChange={value => handleChange(value, 'job')}
            >
              {jobs?.map(item => {
                return (
            ...
            <div style={{ height: 300 }}>
              <Dragger {...draggerProps}>
                ...
                <p className="ant-upload-text">
                  将简历 Word、PDF文件拖拽至此区域
                </p>
                <p className="ant-upload-hint">每个文件大小5M以内</p>
                <p className="ant-upload-hint">
                  目前支持Boss未下载（doc，docx格式），拉勾未下载和已下载（pdf格式），智联未下载和已下载（doc，docx格式），猎聘未下载和已下载（doc，docx格式），前程未下载和已下载（doc，docx格式）
                </p>
              </Dragger>
            </div>
                    ...
                    <Row>
                      ...
                      <Col span={14}>
                        <Progress
                          ...
                          format={() => `${item.uploadStatus}`}
                        />
                      </Col>
                    </Row>
                    <Row
                      style={{
                        display: item.status === 1001 ? 'block' : 'none',
                      }}
                    >
                      <Col>
                        <div style={{ color: 'red' }}>
                            ...
                            ? item.uList.map(item => {
                                return (
                                  <div key={item.resumeId}>
                                    ...
                                    <span
                                      onClick={e => {
                                        checkDetail(item.resumeId);
                                      }}
                                    >
                                      <Button type="link">点此查看</Button>
                                    </span>
                                  </div>
                                );
                              })
